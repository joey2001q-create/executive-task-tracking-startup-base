...
10. 本包内置固定字段结构不可变。建表和校验必须严格使用本文“固定字段清单”中的字段名、类型和选项名，不要主观新增、删减、改名或改类型。
...
字段清单已内置在当前 v6 包内。安装时不要查找、索要或依赖任何历史版本包，只读取以下包内文件和本文字段清单：
...
如果缺表或缺字段，停止安装，报告缺失项。不要向管理员索要 table_id，也不要手工创建非本文固定字段替代。
