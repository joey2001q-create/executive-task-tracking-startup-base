...
`feishu-user-token-registry` 不在本包内。前置授权包完成后，本包负责复制业务模板 Base、校验复制后的业务表，并自动解析业务表 ID。
...
业务表字段结构以模板 Base 为准。复制后只校验，不要主观新增、删减、改名或改类型。不要查找、索要或依赖任何历史版本包。
...
字段结构以业务模板 Base 为准。复制后只校验，不要手动新增、删除、改名或改类型。

字段校验规则：

```text
复制后的模板字段结构必须保持模板复制结果。
不要手动新增、删除、改名或改类型。
字段缺失、类型不一致、公式/查找/关联关系异常时停止安装，报告表名、字段名和差异。
