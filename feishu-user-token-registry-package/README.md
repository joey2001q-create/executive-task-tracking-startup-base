...
- Agent 必须自动转移新 Token 表所有者给管理员。
- 管理员必须手动把 bot 加为新 Token 表“可管理”协作者；Agent 需要暂停提醒，确认后才能继续。
- 不要让管理员手动开启多维表格工作流。
- Agent 应复制模板里已有的所有 workflows，并全部开启为 `Enable`。
- Agent 不创建新的 workflow 定义。
- 安装、模板复制、所有者转移、bot 可管理权限确认、workflow 开启、字段校验、权限校验阶段，不要询问目标成员 open_id。
...
- 把 bot / 应用机器人加入新 Token 表，并设置为“可管理”协作者。
- 飞书应用权限开通。
- OAuth 回调地址配置。
- 应用可用范围配置。
- 应用发布。
